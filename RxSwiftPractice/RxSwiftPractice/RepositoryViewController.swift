...
class RepositoryViewController: UITableViewController{
    // MARK: - Property
    private let baseUrl = "https://api.github.com/orgs/"
    private let targetOrganization = "Apple/repos"
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.title = targetOrganization + "Repository"
        configureTableView()
    ...
    @objc func refresh(){
        DispatchQueue.global(qos: .background).async { [weak self] in
            ...
            self.fetchRepositories(of: self.targetOrganization)
        }
    }
    func fetchRepositories(of organization: String){
        Observable.from([organization])
            .map {organization -> URL in
                return URL(string: self.baseUrl + organization)!
            }
